...
(function($) {
    ...
    WBE.init = function() {
        ...
        console.log('Wootour Bulk Editor initialized');
    };
    ...
    WBE.bindEvents = function() {
        ...
        $(window).on('beforeunload', function(e) {
            if (WBE.state.isProcessing) {
                e.preventDefault();
                e.returnValue = WBE.config.i18n?.confirmLeave || 'Changes may not be saved.';
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    WBE.UI.showError(response.error?.message || 'Unknown error');
    ...
    WBE.UI = {
        ...
        initTooltips: function() {
            // Initialize tooltips using jQuery UI if available
            if ($.fn.tooltip) {
                $('[title]').tooltip();
            }
        },
        ...
        showLoading: function(message) {
            const msg = message || WBE.config.i18n?.loading || 'Loading...';
    ...
    WBE.Products = {
        ...
        handleProductsError: function(error) {
            WBE.UI.showError(error.message || 'Failed to load products');
        },
        
        renderProducts: function(products) {
            if (!products || products.length === 0) {
                this.$productList.html('<div class="wbe-empty-state">' + 
                    (WBE.config.i18n?.noProductsFound || 'No products found') + 
                    '</div>');
                return;
            }
    ...
    // Calendar Module (simplified - will integrate jQuery UI Datepicker)
    WBE.Calendar = {
        init: function() {
            this.cacheElements();
        ...
        initCalendars: function() {
            // This would initialize the custom calendar widgets
            // For now, we're using jQuery UI datepicker
    ...
    WBE.Dates = {
        ...
        updateStartDate: function(date) {
            ...
            if (WBE.state.changes.end_date) {
                ...
                if (start > end) {
                    WBE.UI.showWarning(WBE.config.i18n?.dateConflict || 'Start date cannot be after end date');
                    this.$startDate.val('');
                    delete WBE.state.changes.start_date;
                    return;
                }
        ...
        updateEndDate: function(date) {
            ...
            if (WBE.state.changes.start_date) {
                ...
                if (start > end) {
                    WBE.UI.showWarning(WBE.config.i18n?.dateConflict || 'End date cannot be before start date');
                    this.$endDate.val('');
                    delete WBE.state.changes.end_date;
                    return;
                }
    ...
    WBE.Actions = {
        ...
        validateBeforeAction: function() {
            ...
            if (WBE.state.selectedProducts.size === 0) {
                WBE.UI.showError(WBE.config.i18n?.noProductsSelected || 'Please select at least one product.');
                return false;
            }
            
            // Check if changes are specified
            ...
            if (!hasChanges) {
                WBE.UI.showError(WBE.config.i18n?.noChangesSpecified || 'Please specify at least one change to apply.');
                return false;
            }
        ...
        previewChanges: function() {
            ...
            WBE.ajax('wbe_preview_changes', {
                ...
                WBE.UI.showError(error.message || 'Preview failed');
            });
        },
        ...
        applyChanges: function() {
            ...
            const confirmMessage = WBE.config.i18n?.confirmApply || 
                'Are you sure you want to apply these changes to the selected products?';
            ...
            WBE.UI.updateProgress(0, 'Starting...');
        ...
        handleBatchError: function(error, response) {
            ...
            if (response?.data?.can_resume) {
                ...
                WBE.UI.showInfo(WBE.config.i18n?.resumeAvailable || 'You can resume this operation.');
                $('#wbe-resume-process').show();
            } else {
                WBE.UI.showError(error.message || 'Operation failed');
            }
        },
        ...
        handleBatchComplete: function(data) {
            ...
            const message = WBE.config.i18n?.changesApplied || 'Changes applied successfully.';
            const details = data.success_count + ' products updated, ' + 
            ...
            const pollInterval = setInterval(function() {
                ...
                WBE.ajax('wbe_get_progress', {
                    ...
                    WBE.UI.updateProgress(
                        data.percentage,
                        data.processed + ' of ' + data.total + ' products processed',
        ...
        cancelProcessing: function() {
            ...
            WBE.ajax('wbe_cancel_operation', {
                ...
                WBE.state.isProcessing = false;
                WBE.UI.toggleProgressBar(false);
                WBE.UI.showInfo(WBE.config.i18n?.operationCancelled || 'Operation cancelled.');
            });
        },
        ...
        resetForm: function() {
            ...
            WBE.UI.showInfo(WBE.config.i18n?.formReset || 'Form reset.');
        }
    };
        ...
        showPreview: function(data) {
            ...
            if (data.summary?.has_errors) {
                html += '<div class="notice notice-error"><p>' +
                    (WBE.config.i18n?.previewErrors || 'Errors detected in preview:') +
                    '</p><ul>';
            ...
            WBE.ajax('wbe_get_logs', {}, function(data) {
                ...
                if (data.logs && data.logs.length > 0) {
                    ...
                    html += '<p class="wbe-empty-state">' + 
                        (WBE.config.i18n?.noLogs || 'No logs available.') + '</p>';
                }
